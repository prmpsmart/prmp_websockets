...
class WebSocket:
    def __init__(self, server, sock, address):
        self.server = server
        self.client_address = sock, address
        self.client, self.address = self.client_address
        ...
        if self.opcode == CLOSE:
            ...
            if self.opcode != STREAM:
                if self.opcode == PING or self.opcode == PONG:
                    raise Exception("control messages can not be fragment_ed")
            ...
            if not data:
                ...
                if b"\r\n\r\n" in self.headerbuffer:
                    ...
                    try:
                        ...
                        self._send_buffer(hStr.encode("ascii"), True)
                        self.client.close()
...
class WebSocketServer:
    def __init__(self, host_port:tuple[str, int], websocketclass, selectInterval=0.1):
        host, port = host_port
    ...
    def server_close(self):
        ...
        for conn in self.connections.values():
            conn.close()
            self._handle_closed(conn)

    def _handle_closed(self, client):
        client.client.close()
        # only call handle_closed when we have a successful websocket connection
        if client.handshaked:
            try:
                client.handle_closed()
            except:
                pass
